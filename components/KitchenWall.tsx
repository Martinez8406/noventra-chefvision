...
export const KitchenWall: React.FC<Props> = ({ dishes, onApprove, onOpenTraining, onSelect, onDelete, selectedId }) => {
  ...
  const filterLabels: Record<'all' | 'approved' | 'pending', string> = {
    all: t('filters.all'),
            ...
            <div className="relative h-72 overflow-hidden bg-slate-100">
              ...
              <div className="absolute top-4 right-4 flex flex-col gap-2">
                {dish.status === DishStatus.APPROVED ? (
                  <div className="bg-green-500/90 backdrop-blur-sm text-white p-2.5 rounded-2xl shadow-xl border border-white/20" title={t('status.approvedTitle')}>
                    <CheckCircle2 size={18} strokeWidth={3} />
                  </div>
                ) : (
                  <div className="bg-chef-gold/90 backdrop-blur-sm text-white p-2.5 rounded-2xl shadow-xl border border-white/20 animate-pulse" title={t('status.pendingTitle')}>
            ...
            <div className="p-4 flex-1 flex flex-col justify-between">
              ...
              <div className="mt-2">
                {dish.status === DishStatus.PENDING ? (
                  ...
                  >
                    <CheckCircle2 size={14} /> {t('status.approve')}
                  </button>
                ) : (
        ...
        {filteredDishes.length === 0 && (
          <div className="col-span-full py-24 text-center space-y-4 bg-slate-50 rounded-[48px] border-2 border-dashed border-slate-200">
            ...
            <div className="space-y-1">
              <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-sm">{t('empty.title')}</p>
              <p className="text-slate-300 text-xs font-medium">{t('empty.hint')}</p>
            </div>
          </div>
        )}
      </div>
