...
export async function handlePromoApi({ req, authorization, query, body, method }) {
  const op = resolvePromoOp({ ...req, query: query || req?.query, url: req?.url });
  if (op === 'pin') {
    return handlePromoPin({ req, authorization, query, body, method });
  }
  ...
  return handlePromoCodes({ req, authorization, query, body, method });
}
