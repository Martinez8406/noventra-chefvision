...
export async function requestMenuTranslations(
  dishId: string
): Promise<{ translations: Record<'en' | 'en-us' | 'uk' | 'de' | 'es' | 'it' | 'ko' | 'fr' | 'cs' | 'nl' | 'zh', MenuTranslationEntry> } | null> {
  ...
  const raw = await response.text();
  let data: { translations?: Record<'en' | 'en-us' | 'uk' | 'de' | 'es' | 'it' | 'ko' | 'fr' | 'cs' | 'nl' | 'zh', MenuTranslationEntry>; error?: string } | null = null;
  try {
    data = raw ? JSON.parse(raw) : null;
  } catch {
