...
/**
 * Tworzy sesję Stripe Checkout dla premium | start | tokens | menu_service.
 */
export async function createCheckoutSession({
  ...
  const allowed = new Set(['premium', 'start', 'tokens', 'menu_service']);
  ...
  if (normalizedPlan === 'tokens' || normalizedPlan === 'menu_service') {
    const invoiceDescription =
      normalizedPlan === 'menu_service'
        ? 'Wykonanie menu cyfrowego ChefVision'
        : 'Paczka +50 tokenów AI ChefVision';
