...
export function buildAdminProvisionedClientMessage({
  email,
  restaurantName,
  created = true,
  inviteSent = false,
  ...
  return [
    ...
    'Cena: 0 zł',
    `Zaproszenie: ${inviteSent ? 'wysłane' : 'nie wysłane'}`,
  ].join('\n');
}
...
export function buildImplementationOfferMessage({ kind, email, restaurantName } = {}) {
  ...
  const footer = ['Cena: 0 zł', 'Oferta: Wdrożeniowa', 'Status: Oczekuje na realizację'];

  if (kind === 'bundle') {
    return [
      '🛠️ NOWE ZLECENIE — WDROŻENIE',
      '',
      ...clientLines,
      'Usługi: Wykonanie menu + Ulotka QR',
      ...footer,
